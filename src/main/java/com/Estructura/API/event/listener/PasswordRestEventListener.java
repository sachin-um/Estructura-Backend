...
@Slf4j
public class PasswordRestEventListener implements ApplicationListener<PasswordResetEvent> {
    ...
    public void onApplicationEvent(PasswordResetEvent event) {
        ...
        verificationTokenService.saveVerificationToken(theUser,passwordRestToken,REST_PASSWORD);
        String url=event.getApplicationUrl()+"/api/v1/auth/reset-password?token="+passwordRestToken;
        try {
    ...
    private void sendPasswordRestEmail(String url) throws MessagingException, UnsupportedEncodingException {
