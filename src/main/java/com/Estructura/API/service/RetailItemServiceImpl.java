...
public class RetailItemServiceImpl implements RetailItemService {
    ...
    @Override
    public GenericAddOrUpdateResponse<RetailItemRequest> saveItem(
        RetailItemRequest retailItemRequest) throws IOException {
        GenericAddOrUpdateResponse<RetailItemRequest> response =
            new GenericAddOrUpdateResponse<>();
        if (response.checkValidity(retailItemRequest)) {
            Optional<RetailStore> retailStore = retailStoreService.findById(
            ...
            if (retailStore.isPresent()) {
                RetailItem item = RetailItem.builder()
                                            ...
                                            .description(
                                                retailItemRequest.getDescription())
                                            ...
                                            .quantity(
                                                retailItemRequest.getQuantity())
                                            .build();
                saveImages(retailItemRequest, item);
                RetailItem retailItem = retailItemRepository.save(item);
                String uploadDir = "./files/retail-item-files/" +
                                   retailItem.getRetailStore().getId() +
                                   "/" + retailItem.getId();
                FileUploadUtil.uploadImages(uploadDir,
                                            retailItemRequest.getMainImage(),
                                            retailItem.getMainImageName(),
                                            retailItemRequest.getExtraImages(),
                                            retailItem.getExtraImage1Name(),
                                            retailItem.getExtraImage2Name(),
                                            retailItem.getExtraImage3Name()
                );
    ...
    private void saveImages(RetailItemRequest retailItemRequest,
        RetailItem item) {
        String mainImageName = null;
        if (retailItemRequest.getMainImage() != null &&
            retailItemRequest.getMainImage().getOriginalFilename() != null) {
            mainImageName = StringUtils.cleanPath(
                retailItemRequest.getMainImage().getOriginalFilename());
        }
    ...
    @Override
    public ResponseEntity<List<RetailItem>> getItemsByType(
        RetailItemType type) {
        List<RetailItem> retailItems =
            retailItemRepository.findAllByRetailItemType(
                type);
        if (!retailItems.isEmpty()) {
            return ResponseEntity.ok(retailItems);
        } else {
    ...
    @Override
    public ResponseEntity<List<RetailItem>> getItemsByRetailStore(
        RetailStore retailStore) {
        List<RetailItem> retailItems =
            retailItemRepository.findAllByRetailStore(
                retailStore);
        if (!retailItems.isEmpty()) {
            return ResponseEntity.ok(retailItems);
        } else {
    ...
    public GenericAddOrUpdateResponse<RetailItemRequest> updateItem(
        RetailItemRequest retailItemRequest, Long id) throws IOException {
        GenericAddOrUpdateResponse<RetailItemRequest> response =
            new GenericAddOrUpdateResponse<>();
        if (response.checkValidity(retailItemRequest)) {
            Optional<RetailItem> existingItem = retailItemRepository.findById(
                id);
    ...
    public GenericDeleteResponse<Long> deleteItem(RetailItem retailItem) {
        ...
        Optional<RetailItem> item = retailItemRepository.findById(
            retailItem.getId());
        if (item.isPresent()) {
