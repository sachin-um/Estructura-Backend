...
public class CarpenterServiceImpl implements CarpenterService {
    ...
    public Carpenter saveCarpenter(Carpenter carpenter) {
        Optional<Carpenter> theCarpenter = carpenterRepository.findByEmail(carpenter.getEmail());
        if (theCarpenter.isPresent()) {
            throw new UserAlreadyExistsException("A user with" + carpenter.getEmail() + "already exists");
        }
